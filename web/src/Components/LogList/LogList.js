...
function LogList() {
  ...
  const [logMonth, setLogMonth] = useState(monthsList[new Date().getMonth()]) // DEFAULT LOG MONTH IS CURRENT MONTH
  ...
  function filterLogs(month) {
    ...
    let data = logList.filter(log => {
      if (log.createdAt.split("-")[1] == monthsList.indexOf(month) + 1) {
        return log
      }
    })
    setLogListAll(data)
              ...
              logListAll.map((el, i) => {
                return (
                  <div className='log-card' key={i}>
                    <p className='log-date'>{el.createdAt.split("T")[0]}</p>
